...
document.addEventListener("DOMContentLoaded", function(event) {
	...
	scoreboard_svg = d3.select("#scoreboardframe")
	.append("svg:svg")
	.attr("id", "scoreboardgraph"); 
	// .attr("class", "hidden"); // only make visible if data loaded

	scoreboard = scoreboard_svg.append("g")
	.attr("id", "scoreboard")
	.attr("class", "score");

	var cmd = 'getGameInfo';
	var url = base_url + '?token=' + token + '&appid=' + appid + '&cmd=' + cmd ;
	// console.log(url);
	d3.json(url, function(error, data) {
		...
		if (typeof data != 'undefined' || data.playerdata !== null) {
			...
			now = new Date();
			// console.log('Start:'+now.getHours()+':'+now.getMinutes()+':'+now.getSeconds()+' Uhr');
...
function updateEvents() {
	var diff = max_event_id - event_id;
	// console.log(diff);
...
function getTickerCount(currCount, callbackFunc) {
	...
	var url = base_url + '?token=' + token + '&appid=' + appid + '&cmd=' + cmd + '&index=' + currCount;
	// console.log(url);
	var xhttp;
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			try {
				...
				// throw 'jsonParseException'; // generates an exception
			}
			catch (e) {
	...
	for (let i = 0; i < eventList.length; i++) {
		...
		if (eventList[i].valid) {
			...
			if (testMode) {
				...
				// if (!testMode) cell.classList.add('hidden');
...
function updatePlayerDisplay(list, team) {
	...
	// cell.appendChild(document.createTextNode('Spieler')); 
	cell.appendChild(document.createTextNode('')); 
	row.appendChild(cell); 
...
function parseEvent(event) {
	...
	event.guest_score = event.guest_score * 1;
	...
	re = /Nummer (\d{1,2}|[A-D])/i;
	// TODO Mannschaftsverantw.
	temp = event.message.match(re);
	event.player = null;
	if (temp !== null) event.player = temp[1];
...
function addToPlayerList(event) {
	...
	// console.log(id);
	if (id === list.length) {
		list.push({"player": event.player, "goals":0, "goals7m":0, "penalty":0, "yellow":0, "suspension":0, "red":0});
	} 
...
function initializeScoreboard() {
	...
	scoreboard_svg
		.attr("width", sb_width+"px")
		.attr("height", (sb_height+sb_vert_margin*2)+"px");
	...
	scoreboard.append("rect")
		...
		.attr("x", sb_offset + "px")
		.attr("y", sb_offset + "px")
		.attr("width", (sb_width - 2 * sb_offset) + "px")
		.attr("height", (sb_height - 2 * sb_offset) + "px")
		.attr("rx", sb_radius)
		.attr("ry", sb_radius)
		.text("0");
	scoreboard.append("rect")
		.attr("class", "background")
		.attr("x", (2 * sb_offset) + "px")
		.attr("y", (2 * sb_offset) + "px")
		.attr("width", (sb_width - 4 * sb_offset) + "px")
		.attr("height", (sb_height - 4 * sb_offset) + "px")
		.attr("rx", sb_radius)
		.attr("ry", sb_radius)
		.text("0");
	...
	scoreboard.append("text")
		...
		.attr("x", (sb_width / 2 + 46) + "px")
		.attr("y", sb_time + "px")
		.text("88 88");

	scoreboard.append("text")
		.attr("class", "diget dots bg")
		.attr("x", (sb_width / 2) + "px")
		.attr("y", sb_time + "px")
		.text(":");
...
function updateScoreBoard(score) {
	...
	.text(score.home_score);
	scoreboard.select("#scoreAway")
	.text(score.guest_score);
	scoreboard.select("#scoreTime")
	.text(formatTime(score.game_time).replace(':', ' '));
}
